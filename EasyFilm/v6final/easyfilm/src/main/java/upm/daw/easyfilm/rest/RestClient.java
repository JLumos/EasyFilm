...
public class RestClient {
     ...
     */
    private Pelicula buscarEnDb(String nombre) {return peliculaRepository.findByNombrePelicula(nombre);}
    ...
    public Pelicula getPelicula(String nombre) throws IOException {
                ...
                || pelicula.getValoracion() == null)
        {
            ...
            {
                //Usando el ID de Imdb se obtiene el ID de TMDB
                url2 = "https://api.themoviedb.org/3/find/" + pelicula.getImdbId() + keyTMDB + "&external_source=imdb_id";
                InfoResult info = restTemplate.getForObject(url2,InfoResult.class);
                if (info.getMovieResults().size() > 0) {
                    pelicula.setTmdbId(info.getMovieResults().get(0).getId());
                    if (pelicula.getTmdbId() != null && !pelicula.getTmdbId().equals("")) {
                        //Usando el ID de Tmdb se obtiene el trailer (si existe)
                        url2 = "https://api.themoviedb.org/3/movie/" + pelicula.getTmdbId() + "/videos" + keyTMDB;
                        InfoResultTrailer trailer = restTemplate.getForObject(url2, InfoResultTrailer.class);
                        if (trailer.getResults().size() > 0) {
                            String embed = trailer.getResults().get(0).getKey();
                            if (embed != null && !embed.equals(""))
                                pelicula.setUrl_trailer("https://www.youtube-nocookie.com/embed/" + embed);
                        }
                    }
                }
            }
